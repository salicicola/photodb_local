...
## XXX BUG :: /media/data/data/django4/photodb/edit.py, line 420, in identify famname = family.latname
## so far 
## manually assigned to family level and only for animals
## in entry:: manually added items for generic family
## not yet for genus (will need *3 files?
...
def view_photo_records(request, pnid=None, lcid=None, exclude=None, thumbs=True, view="gallery", template="photodb/photos/gallery_index.htm", save=False, legacy=False, public=True, lcid_starts=None, fid=None):
    ...
    if pnid:
        ...
        if rank == 'group':
            ...
            grp = ["", "", "", group.pnid, [], 0]
            tree.append(grp)
            ## XXX start test pnid = 17041/
            taxon = Name.objects.get(pnid=pnid)
            famphotos = AnimalImage.objects.filter(plant=taxon)
            print ("XXX", taxon, famphotos)
            
            try:
                fam = [family.latname.replace('_', ' '), family.colnames, family.authors, family.pnid, [], 0, famphotos]
            except:
                fam = [family.sal_latname.replace('_', ' '), family.colnames, family.sal_authors, family.pnid, [], 0, famphotos]
            ## XXX ends
            print ("pass fam", fam)
            grp[4].append(fam)
            for genus in Name.objects.filter(upper=family).order_by("latname"):
